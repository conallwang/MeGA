	...
	const bool debug)
{
  ...
  torch::Tensor out_near_z = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor out_near_z2 = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor out_near_z3 = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_near_z.contiguous().data<float>(),
		out_near_z2.contiguous().data<float>(),
		out_near_z3.contiguous().data<float>(),
		out_num.contiguous().data<int>(),
		depth_scale_factor,
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_near_z, out_near_z2, out_near_z3, out_num, radii, geomBuffer, binningBuffer, imgBuffer);
}
