...
class NVDiffRenderer(torch.nn.Module):
    ...
    def shade(self, normal, lighting_coeff=None):
        if self.lighting_type == "constant":
            ...
            diffuse = torch.clamp(
                V.dot(normal, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device="cuda")), 0.0, 1.0
            )
        else:
    ...
    def render_from_camera(
        ...
        for cam in cameras:
            ...
            RT.append(world_view_transform.T[None, ...])

            full_proj_transform = cam.full_proj_transform.clone()
            full_proj_transform[:, 1] = -full_proj_transform[:, 1]
            full_proj.append(full_proj_transform.T[None, ...])
        RT = torch.cat(RT, dim=0)
        full_proj = torch.cat(full_proj, dim=0)

        if self.use_opengl:
            image_size = self.img_h, self.img_w

            return self.render_mesh(verts, faces, RT, full_proj, image_size, attr, background_color)
        else:
            if self.img_h > 2048 or self.img_w > 2048:
                image_size = 2048, 2048
            else:
                image_size = upTo8Multiples(self.img_h), upTo8Multiples(self.img_w)

            output = self.render_mesh(verts, faces, RT, full_proj, image_size, attr, background_color)
            for k, v in output.items():
                output[k] = v[:, : self.img_h, : self.img_w]
                # output[k] = F.interpolate(v.permute(0, 3, 1, 2), (self.img_h, self.img_w), mode="bilinear").permute(
                #     0, 2, 3, 1
                # )
            return output
    ...
    def render_mesh(
        ...
        return {
            ...
            "rgba": rgba_aa.flip(1),
            "neural_img": rast_attr[..., 1:].flip(1),
            "depth": rast_attr[..., 0].flip(1) * 1000.0,
            "valid_nograd": fg_mask[..., 0].flip(1),
            "face_id": rast_out[..., -1].long().flip(1) - 1,
            "face_bw": rast_out[..., :2].flip(1),
        }
